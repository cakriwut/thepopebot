...
async function* chatStream(threadId, message, attachments = [], options = {}) {
  ...
  try {
    ...
    for await (const event of stream) {
      ...
      if (msgType === 'ai') {
        // Track web search across streaming chunks
        if (Array.isArray(msg.content)) {
          for (const block of msg.content) {
            if (block.type === 'server_tool_use' && block.name === 'web_search' && block.input) {
              webSearchInput += block.input;
            }
            if (block.type === 'web_search_tool_result') {
              let query = '';
              if (webSearchInput) {
                try { query = JSON.parse(webSearchInput).query; } catch {}
              }
              console.log(`[chatStream] Web search${query ? ` query: "${query}"` : ' executed'}`);
              webSearchInput = '';
            }
          }
        }
